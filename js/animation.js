...
function startAnimation() {
  ...
  // Hide all elements except the logo during animation
  document.querySelectorAll('.menu-bar, .skyline-container, #search-icon-container, #hover-text')
    .forEach(el => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // After logo settles, fade in other elements
        setTimeout(() => {
          // Show other elements with fade-in effect
          document.querySelectorAll('.menu-bar, .skyline-container, #search-icon-container')
            .forEach(el => {
          ...
          if (window.innerWidth >= 1024) {
            if (hoverText) {
              ...
              hoverText.classList.add('fade-in');
...
window.addEventListener('load', () => {
  ...
  if (!document.body.classList.contains('subpage')) {
    ...
    document.querySelectorAll('.word.art, .word.life, .subpage-word.art, .subpage-word.life')
      .forEach(w => { w.style.transform = 'none'; w.style.opacity = '1'; });
  }
});
...
class Typewriter {
  ...
  type() {
    if (!this.isTyping || !this.isLargeScreen) return;
